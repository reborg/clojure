...
//		         (isVariadic() ? "clojure/lang/RestFn" : "clojure/lang/AFunction"), null);
        String source = (String) clojure.lang.Compiler.SOURCE.deref();
        ...
        Compiler.addAnnotation(cv, objExpr.classMeta);
...
*/
        ...
        Method m = new Method("<init>", Type.VOID_TYPE, objExpr.ctorTypes());
...
//			}
        ...
        if (objExpr.altCtorDrops > 0) {
            ...
            ctorgen.invokeConstructor(objExpr.objtype, new Method("<init>", Type.VOID_TYPE, ctorTypes));
        ...
        if (objExpr.supportsMeta()) {
            ...
            gen = new GeneratorAdapter(Opcodes.ACC_PUBLIC,
                    ...
                    cv);
            gen.visitCode();
            gen.newInstance(objExpr.objtype);
            gen.dup();
            gen.loadArg(0);
            ...
            for (ISeq s = RT.keys(objExpr.closes); s != null; s = s.next(), ++a) {
                ...
                Class primc = lb.getPrimitiveType();
                if (primc != null) {
                    gen.getField(objExpr.objtype, lb.name, Type.getType(primc));
                } else {
                    gen.getField(objExpr.objtype, lb.name, Compiler.OBJECT_TYPE);
                }
            }

            gen.invokeConstructor(objExpr.objtype, new Method("<init>", Type.VOID_TYPE, ctorTypes));
            gen.returnValue();
            gen.endMethod();
        }
