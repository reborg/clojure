                ...
                if (RT.booleanCast(RT.WARN_ON_REFLECTION.deref())) {
                    RT.errPrintWriter()
                            .format("Reflection warning, %s:%d:%d - call to method %s on %s can't be resolved (no such method).\n",
                                    Compiler.SOURCE_PATH.deref(), line, column, methodName, target.getJavaClass().getName());
                }
            } else {
                int methodidx = 0;
                if (methods.size() > 1) {
                ...
                if (method == null && RT.booleanCast(RT.WARN_ON_REFLECTION.deref())) {
                    RT.errPrintWriter()
                            .format("Reflection warning, %s:%d:%d - call to method %s on %s can't be resolved (argument types: %s).\n",
                                    Compiler.SOURCE_PATH.deref(), line, column, methodName, target.getJavaClass().getName(), Compiler.getTypeStringForArgs(args));
                }
            }
        } else {
            method = null;
